 ...
 * limitations under the License.
 ...
 this.calcFindWordMatch(query, pageIndex, pageContent); // 覆盖phrasematch
 
 */
var outline_info_array = [];
var window_size = 1;
...
var PDFViewerApplication = {
  ...
  setTitle: function setTitle(title) {
    ...
    // document.title = title;//2019-5-22 17:06:25
  },
  ...
  load: function load(pdfDocument, scale) {
    ...
    Promise.all([onePageRendered, _ui_utils.animationStarted]).then(function () {
      pdfDocument.getOutline().then(function (outline) {
        _this6.pdfOutlineViewer.render_pseudo({ outline: outline });
      });
...
var PDFLinkService = function () {
  ...
  _createClass(PDFLinkService, [{
    ...
    key: 'navigateTo_pseudo',
    value: function navigateTo_pseudo(dest, node_ref, pgn_resolved) {
      var _this = this;
      ...
      var goToDestination = function goToDestination(_ref2) {
        ...
        if (destRef instanceof Object) {
          // console.log('destRef of _this._cachedPageNumber(destRef);',destRef)//2019-5-2 16:55:26
          pageNumber = _this._cachedPageNumber(destRef);
          // console.log('pageNumber upper is',pageNumber)//2019-5-2 16:23:57
          if (pageNumber === null) {
        ...
        // console.log('pageNumber lower is',pageNumber)//2019-5-2 16:23:57
        return pageNumber
      };
      //2019-5-2 20:48:06
      // var page_num;
      new Promise(function (resolve, reject) {
        ...
        if (!(data.explicitDest instanceof Array)) {
          node_ref.page_num = NaN//goToDestination(data);//2019-5-2 21:19:48//2019-5-28 23:38:54
          pgn_resolved();
          console.log('the invalid data:',data)
          // console.log('PDFLinkService.navigateTo: "' + data.explicitDest + '" is' + (' not a valid destination array, for dest="' + dest + '".'))
          console.error('PDFLinkService.navigateTo: "' + data.explicitDest + '" is' + (' not a valid destination array, for dest="' + dest + '".'));//error surppressed//2019-5-28 23:44:04
          return;//2019-5-28 23:45:06
        }
        node_ref.page_num = goToDestination(data);//2019-5-2 21:19:48
        pgn_resolved();
      });
    // return page_num;
    }    
  ...
  _createClass(PDFFindController, [{
    ...
    value: function _prepareMatches(matchesWithLength, matches, matchesLength) {
      function isSubTerm(matchesWithLength, currentIndex) {
        ...
        for (var i = currentIndex - 1; i >= 0; i--) {
          ...
          if (prevElem.match + prevElem.matchLength >= currentElem.match) {
            var currentElem_match_original = currentElem.match;
            currentElem.match = prevElem.match + prevElem.matchLength;
            currentElem.matchLength -= prevElem.match + prevElem.matchLength - currentElem_match_original;
          }
        }
        return false;
      }
    ...
    value: function calcFindWordMatch(query, pageIndex, pageContent) {
      ...
      for (var i = 0, len = queryArray.length; i < len; i++) {
        ...
        var subquery_for_regex = subquery.split('_').join(' {0,2}')
        var subquery_re = new RegExp(subquery_for_regex,'i')
        var subquery_match = pageContent.match(subquery_re)
        if(subquery_match!=null){
          var subquery_for_indexOf = subquery_match[0]
        }else{
          var subquery_for_indexOf = subquery.replace(/_/g,' ')
        }
        // subquery = subquery.replace(/_/g,' ');
        // console.log('subquery is: ',subquery);
        // var subqueryLen = subquery.length;
        var subqueryLen = subquery_for_indexOf.length;
        var matchIdx = -subqueryLen;
        while (true) {
          matchIdx = pageContent.indexOf(subquery_for_indexOf, matchIdx + subqueryLen);
          if (matchIdx === -1) {
            break;
          }
    ...
    value: function extractText() {
      ...
      var start_idx = window.PDFViewerApplication.page - 1 - window_size;
      start_idx = (start_idx >=0)? start_idx:0;
      var stop_before_idx = window.PDFViewerApplication.page -1 + window_size + 1;
      var ii = this.pdfViewer.pagesCount;

      var i_list = [];
      for (var i = start_idx, ii = stop_before_idx; i < ii; i++) {
        i_list.push(i);
      }
      for (var i = 0, ii = start_idx; i < ii; i++) {
        i_list.push(i);
      }
      for (var i = stop_before_idx, ii = this.pdfViewer.pagesCount; i < ii; i++) {
        i_list.push(i);
      }

      for (var j of i_list) {
        i = Number(j);
        //console.trace(i,'frome _loop')
        _loop(i, ii);
      }
    }
  }, {
...
var PDFOutlineViewer = function () {
  ...
  _createClass(PDFOutlineViewer, [{
    ...
    key: '_bindLink_pseudo',
    value: function _bindLink(element, item, node_ref, pgn_resolved) {
      var _this = this;
      ...
      _this.linkService.navigateTo_pseudo(destination, node_ref, pgn_resolved);//2019-5-2 16:23:57
      ...
      // return page_num;//2019-5-2 20:45:17
    }
  }, {
    key: '_bindLink',
    value: function _bindLink(element, item) {
      ...
      // console.log('destination',destination)
      element.href = this.linkService.getDestinationHash(destination);
      // _this.linkService.navigateTo_pseudo(destination);//2019-5-2 16:23:57
      element.onclick = function () {
        if (destination) {
          _this.linkService.navigateTo(destination);
        }
    ...
    key: 'render_pseudo',
    value: function render_pseudo(_ref2) {
      ...
      var all_page_num_promise_list = [];//2019-5-3 22:26:52
      while (queue.length > 0) {
        ...
        for (var i = 0, len = levelData.items.length; i < len; i++) {
          ...
          var node_ref = {'title':null,
                          'page_num':null
                          }//2019-5-2 21:10:44
          var pgn_promise = new Promise(pgn_resolved=>{
            // var pgn_resolved=r;
            this._bindLink_pseudo(element, item, node_ref, pgn_resolved);//2019-5-2 20:45:01           
          });
          all_page_num_promise_list.push(pgn_promise);
          this._setStyles(element, item);
          element.textContent = (0, _pdfjsLib.removeNullCharacters)(item.title) || DEFAULT_TITLE;
          node_ref.title = element.textContent;//2019-5-2 21:10:51
          // console.log(element.textContent, page_num)//2019-5-2 16:23:57

          if ('slot' in levelData){
            // console.log('should insert');
            levelData['slot'].push(node_ref);
          }else{
            outline_info_array.push(node_ref);
          };
          ...
          if (item.items.length > 0) {
            ...
            if ('slot' in levelData){
            // console.log('should insert');
              levelData['slot'].push(to_be_appended);
            }else{
              outline_info_array.push(to_be_appended);
            };
      ...
      this._dispatchEvent(outlineCount);
    Promise.all(all_page_num_promise_list).then(function(){
      // console.log(outline_info_array);
      outline_resolver('from viewr.js outline..')
    })
    // setTimeout(function(){
    //   // console.log(outline_info_array);
    //   get_level_from_nested_outline(outline_info_array,0)
    //   console.log('flat_outline_array',flat_outline_array)
    // },1000)
    }
  }, {
    key: 'render',
    value: function render(_ref2) {
      ...
      while (queue.length > 0) {
        ...
        for (var i = 0, len = levelData.items.length; i < len; i++) {
          ...
          this._setStyles(element, item);
          element.textContent = (0, _pdfjsLib.removeNullCharacters)(item.title) || DEFAULT_TITLE;
          // console.log(element.textContent)//2019-5-2 16:23:57
          div.appendChild(element);
          if (item.items.length > 0) {
            hasAnyNesting = true;
            this._addToggleButton(div);
